...
class Tunnel_DataLoader(Dataset):
    """Tunnel dataset loader."""
    def __init__(
            self,
            subset,
            data_path,
        ...
        # TODO need to handle multiple videos
        self.rawVideoExtractor = RawVideoExtractor(framerate=feature_framerate, size=image_resolution,slice_framepos=slice_framepos,max_frames=max_frames*(self.cut_off_points[0]+1))
        ...
        self.video_data_pre = None
        self.video_mask_pre = None
        self.video_id_pre   = None
    ...
    def _get_rawvideo(self, choice_video_ids):
        vid_len = self.max_frames*(self.cut_off_points[0]+1)
        video_mask = np.zeros((len(choice_video_ids), vid_len), dtype=np.long)
        max_video_length = [0] * len(choice_video_ids)

        # Pair x L x T x 3 x H x W
        video = np.zeros((len(choice_video_ids), vid_len, 1, 3,
        ...
        for i, video_id in enumerate(choice_video_ids):
            ...
            if len(raw_video_data.shape) > 3:
                ...
                if vid_len < raw_video_slice.shape[0]:
                    if self.slice_framepos == 0:
                        video_slice = raw_video_slice[:vid_len, ...]
                    elif self.slice_framepos == 1:
                        video_slice = raw_video_slice[-vid_len:, ...]
                    else:
                        sample_indx = np.linspace(0, raw_video_slice.shape[0] - 1, num=vid_len, dtype=int)
    ...
    def __getitem__(self, idx):
        ...
        if  video_id == self.video_id_pre:
            video = self.video_data_pre
            video_mask = self.video_mask_pre
        else:
            self.video_id_pre = video_id
            video, video_mask = self._get_rawvideo(choice_video_ids)
            self.video_data_pre = video
            self.video_mask_pre = video_mask
...
if __name__=='__main__':
    ...
    tunnel_dataloader = Tunnel_DataLoader\
    (
        subset='test',
        data_path='/scratch2/kat049/Git/InternVideo/Downstream/Video-Text-Retrieval/data/Tunnel',#'/scratch2/kat049/tmp/bear/camera0-1024x768-000.mp4',
        features_path='/scratch2/kat049/tmp/bear',#'/scratch2/kat049/Git/STVT/STVT/STVT/datasets/datasets/bear_camera0-1024x768-001.h5',
        tokenizer=tokenizer,
        max_words=30,
        feature_framerate=1,
        max_frames=600,
    ...
    for bid, batch in enumerate(tunnel_dataloader):
